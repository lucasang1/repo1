...
fig = px.scatter(
    ...
    title = "Non-Penalty Goal Conversion % vs Total Shots",
)
...
fig.update_traces(
    hovertemplate=(
        "<b>%{hovertext}</b><br>" +
        "%{customdata[0]}<br>" +
        "<extra>Non-Penalty Goal Conversion: %{y}%<br>"
        "Total Shots: %{x}<br>" +
        "%{customdata[1]}</extra>"
    ),
    marker = dict(size = 10)
...
fig.add_vline(
    ...
    line_color = "gold",
    line_width = 1,
    annotation_text = f"90th Percentile ({x90:.0f} Shots)",
...
fig.update_layout(
    ...
    xaxis = dict(range = [0, df[x].max() * 1.02], title = "Total Shots"),
    yaxis = dict(range = [-1, df[y].max() * 1.1], title = "Non-Penalty Goals Conversion %"),
    margin = dict(l = 60, r = 40, t = 80, b = 60),
    legend = dict(font = dict(size = 10))
    ) 

fig.show()
fig.write_html("nPGCvsTotShots_interactive.html")